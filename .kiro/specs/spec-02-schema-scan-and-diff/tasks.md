...
- [ ] 1. 建立“当前 schema”持久化模型与扫描任务运行时上下文模型
- [ ] 1.1 基于现有 `ldb_table_schemas` / `ldb_column_schemas` 调整“当前 schema”覆盖更新语义，并维护扫描任务运行时状态，不新增扫描任务持久化表、快照表或审计表。
  - 增加必要状态字段（最后扫描时间、最后同步时间、`schema_trust_state`、最近阻断原因）。
...
- [ ] 2. 实现扫描编排与内存快照构建
- [ ] 2.1 实现全库扫描流程：按确定性顺序读取表、列、主键、唯一约束与外键，并构建统一内存 schema 图。
  - 支持多方言抽象，保证输出结构一致。
