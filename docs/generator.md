...
- AI/LLM 生成能力不纳入 MVP（保留 schema 与扩展位，不进入默认流程）。
- 计算字段在 MVP 阶段仅支持 SQL 表达式；Python 表达式延期是出于阶段复杂度控制，不代表方案不可行。
- 全局 seed 为 MVP 能力，用于复现测试与问题定位。
- 性能口径固定为：不包含外部数据源调用、不包含计算字段、包含唯一性约束检查、事务基准为每批事务。
...
### 3.3 唯一性与唯一索引冲突语义（批量插入）

- `CommonOptions.Unique=true` 默认表示**整个目标表唯一**（不是仅“本次生成”唯一）。
- 当启用表级唯一时，`UniquePool` 在任务开始前必须先初始化，并加载目标表该列（或联合唯一键相关列）已存在值，再参与后续生成去重。
- 唯一性在内存中保证：生成阶段通过 `UniquePool` 做预检查与登记，避免批量写入时产生可预见重复值。
- 若写入目标库时仍发生唯一索引冲突（例如并发写入导致竞态），任务按失败处理：**立即中断后续任务并回滚已写入数据**。
...
└── 通用生成器
    ...
    ├── SQL 计算表达式 - 可以引用其它字段
    ├── Python 计算表达式 - 可以引用其它字段（非 MVP，后续阶段）
    ├── 外键关联 - 从关联表取值
    ├── 自引用 - 树结构父ID
    ├── 外部数据源 - SQL/JSON文件/REST API
    └── AI生成 - LLM 生成语义化内容（非 MVP，后续阶段）
```
...
- SQL计算表达式和Python计算表达式: 可以引用其它字段。MVP 仅启用 SQL 计算表达式；Python 表达式方案已明确并保留到后续阶段。表达式的计算在所有字段数据生成后才开始计算，如果一条语句中包含多个表达式生成器。则需要评估字段生成顺序
...
- 批量生成性能测试：≥10,000 条/秒（口径：不包含外部数据源与计算字段，包含唯一性约束检查，采用每批事务）
